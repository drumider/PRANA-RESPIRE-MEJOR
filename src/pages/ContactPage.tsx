      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 mb-16">
          ...
          <div className="bg-white rounded-3xl p-8 border border-slate-200/80 shadow-lg hover:shadow-xl transition-all flex flex-col justify-between">
            <div>
              ...
              <div className="space-y-4 mb-8 text-sm text-slate-700">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <strong className="block text-slate-900 font-semibold">{isEs ? 'Teléfono Directo:' : 'Direct Phone:'}</strong>
                    <a href={`tel:${LOCATIONS.catolica.phoneRaw}`} className="text-cyan-700 font-bold hover:underline">
                      {LOCATIONS.catolica.phone}
                    </a>
                  </div>
                </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-slate-100">
              <a
                ...
                className="flex-1 inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl font-bold text-sm bg-slate-100 hover:bg-slate-200 text-slate-800 transition-colors"
              >
                <span>{isEs ? 'Ver en Waze / Maps' : 'View on Maps'}</span>
                <ExternalLink className="w-4 h-4 text-slate-600" />
              </a>

              <a
                href={`tel:${LOCATIONS.catolica.phoneRaw}`}
                className="flex-1 inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl font-bold text-sm bg-cyan-600 hover:bg-cyan-700 text-white transition-colors"
              >
                <Phone className="w-4 h-4" />
                <span>{isEs ? 'Llamar a Sede' : 'Call Office'}</span>
              </a>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="space-y-4 mb-8 text-sm text-slate-700">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <strong className="block text-slate-900 font-semibold">{isEs ? 'Teléfono Directo:' : 'Direct Phone:'}</strong>
                    <a href={`tel:${LOCATIONS.metropolitano.phoneRaw}`} className="text-cyan-700 font-bold hover:underline">
                      {LOCATIONS.metropolitano.phone}
                    </a>
                  </div>
                </div>
        ...
        <div className="bg-gradient-to-r from-emerald-600 to-teal-700 rounded-3xl p-8 sm:p-10 text-white shadow-xl flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4 shrink-0 w-full md:w-auto">
            <a
              href={isEs ? LOCATIONS.whatsapp.link : LOCATIONS.whatsapp.linkEn}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-2.5 px-6 py-3.5 rounded-xl font-extrabold text-slate-900 bg-white hover:bg-emerald-50 transition-all shadow-md text-sm"
            >
              <MessageCircle className="w-5 h-5 text-emerald-600 fill-emerald-600" />
              <span>WhatsApp: (506) 8828-2904</span>
            </a>

            <button
              onClick={onOpenBooking}
              className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl font-extrabold text-white bg-slate-900 hover:bg-slate-800 transition-all shadow-md text-sm cursor-pointer"
            >
